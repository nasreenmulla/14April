...
const AddPayee = ({ addBank }) => {
  ...
  const fetchBankEntries = async () => {
    try {
      const response = await api.get(`/api/PayeeEntries?pageSize=${pageSize}&searchQuery=${searchQuery}`);
  ...
  const handleADD = async (e) => {
    e.preventDefault();
    try {
        const data = await api.post("/createP", formData);
        console.log(data);
  ...
  const handleDelete=async(id)=>{
    const data=await api.delete("/deleteP/"+id)
    // alert(data.data.message)
    if(data.data.success){
      fetchBankEntries();
      alert(data.data.message)
  ...
  const handleUpdate=async(e)=>{
    e.preventDefault();
    const data=await api.put("/updateP/",formDataEdit)
    if(data.data.success){
      fetchBankEntries();
      alert(data.data.message)
